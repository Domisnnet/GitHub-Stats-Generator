...
class DashboardGenerator:
    ...
    def _draw_languages_bar(self, max_langs=7):
        ...
        if not self.langs or self.total_repos == 0:
            lang_ax = inset_axes(self.ax, width="94%", height="75%", bbox_to_anchor=(0.05, 0.16, 0.90, 0.38), bbox_transform=self.ax.transAxes, loc="center")
        ...
        lang_ax.set_xticks([])
        lang_ax.tick_params(axis="y", length=0, labelsize=14, pad=0)
        for bar in bars:
            width = bar.get_width()
